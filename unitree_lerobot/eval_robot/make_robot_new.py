...
import threading
import torch
from unitree_lerobot.eval_robot.teleimager.src.teleimager.image_client import ImageClient
...
def setup_image_client(args: argparse.Namespace) -> ImageClient:
    """Initializes and starts the image client."""
    if getattr(args, "sim", False):
        return ImageClient(host="127.0.0.1")
    else:
        return ImageClient()
...
def process_images_and_observations(image_client: ImageClient, arm_ctrl):
    """Processes images and generates observations."""    
    cam_config = image_client.get_cam_config()

    left_top_cam = None
    if cam_config['head_camera']['enable_zmq']:
        left_top_cam = image_client.get_head_frame()[0]
    
    left_wrist_cam = None
    if cam_config['head_camera']['enable_zmq']:
        left_wrist_cam = image_client.get_left_wrist_frame()[0]
    
    right_wrist_cam = None
    if cam_config['head_camera']['enable_zmq']:
        right_wrist_cam = image_client.get_right_wrist_frame()[0]
